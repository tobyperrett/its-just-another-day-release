...
def get_args_parser():
    ...
    parser.add_argument('--root', default='/jmain02/home/J2AD001/wwp01/txp48-wwp01/ego4d_data/v2/video_256_30fps_300s', type=str, help='path to dataset root')
    # parser.add_argument('--root', default='/jmain02/home/J2AD001/wwp01/txp48-wwp01/ego4d_data/v2/video_384_30fps_300s', type=str, help='path to dataset root')
    parser.add_argument('--chunk_len', default=300, type=int, help='chunk length of mp4s')
    parser.add_argument('--metadata', default='/jmain02/home/J2AD001/wwp01/txp48-wwp01/lavila_output/L336/evl_recall_10_fcl5.0/captions/train_preds.pkl', type=str, help='path to metadata file')
    parser.add_argument('--metadata_sets', default='/jmain02/home/J2AD001/wwp01/txp48-wwp01/lavila_output/L336/comb_prompt/evl_recall_tfl_10_fcl5.0/amin_clip_P1_OnlinePerVid_p10_e0.pkl', type=str, help='path to metadata file')
    # parser.add_argument('--metadata_sets', default='/jmain02/home/J2AD001/wwp01/txp48-wwp01/lavila_output/L336/evl_recall_10_fcl5.0/captions/train_preds.pkl', type=str, help='pkl file containing sets of similar videos')
    # parser.add_argument('--metadata_sets', default="/jmain02/home/J2AD001/wwp01/txp48-wwp01/lavila_input/val_recall_sets.pkl", type=str, help='pkl file containing sets of similar videos')
    parser.add_argument('--output-dir', default='/jmain02/home/J2AD001/wwp01/txp48-wwp01/lavila_output/L336/comb_prompt/evl_recall_10_fcl5.0-test', type=str, help='output dir')
    ...
    # parser.add_argument('--resume', default='/jmain02/home/J2AD001/wwp01/txp48-wwp01/lavila_ckpts/clip_openai_timesformer_base.narrator_rephraser.ep_0005.md5sum_d73a9c.pth', type=str, help='path to latest checkpoint')
    parser.add_argument('--emb_resume', default='/jmain02/home/J2AD001/wwp01/txp48-wwp01/lavila_ckpts/clip_openai_timesformer_large.narrator_rephraser.ep_0003.md5sum_c89337.pth', type=str, help='path to latest checkpoint')
    parser.add_argument('--cap_resume', default='/jmain02/home/J2AD001/wwp01/txp48-wwp01/lavila_ckpts/vclm_openai_timesformer_large_336px_gpt2_xl.pt_ego4d.jobid_246897.ep_0003.md5sum_443263.pth', type=str, help='path to latest checkpoint')
    # parser.add_argument('--cap_resume', default='/jmain02/home/J2AD001/wwp01/txp48-wwp01/lavila_ckpts/vclm_openai_timesformer_base_gpt2_base.pt_ego4d.jobid_319630.ep_0002.md5sum_68a71f.pth', type=str, help='path to latest checkpoint')
    # parser.add_argument('--pp_resume', default='/jmain02/home/J2AD001/wwp01/txp48-wwp01/lavila_output/L336/comb_prompt/evl_recall_tfl_10_fcl5.0/amin_clip_P1_OnlinePerVid_e40_model.pkl', type=str, help='path to latest checkpoint')
    parser.add_argument('--pp_resume', default='/jmain02/home/J2AD001/wwp01/txp48-wwp01/lavila_output/L336/comb_prompt/evl_recall_tfl_10_fcl5.0/cs64-0.001-s20-30_CSPair_CSPairLoss_p10_e40_model.pkl', type=str, help='path to latest checkpoint')
    ...
    parser.add_argument('--load_similar_videos_range', default=[9], help="one int for constant, two ints for range")
    ...
    # parser.add_argument('--root_cap', default='/jmain02/home/J2AD001/wwp01/txp48-wwp01/lavila_output/GPT2-TF5-8iq-5s-v2', type=str, help='path to dataset root')
    # parser.add_argument('--sub_root', type=str, default='evl_recall_10_fcl5.0/narration_emb')
    # parser.add_argument('--root_cap', default='/jmain02/home/J2AD001/wwp01/txp48-wwp01/lavila_output/L336', type=str, help='path to dataset root')
    # parser.add_argument('--sub_root', type=str, default='evl_recall_10_fcl5.0/captions')
    # parser.add_argument('--load_prompts', default=[i for i in range(10)], type=int, help="num_img_queries in narrator")
    # parser.add_argument('--root_vis', type=str, default='/jmain02/home/J2AD001/wwp01/txp48-wwp01/lavila_output/GPT2-TF5-8iq-5s-v2/evl_recall_10_fcl5.0/clip_emb_narration')
    ...
    parser.add_argument('--n_sets', default=100, type=int, help="stop after a certain number of sets")
    # parser.add_argument('--max_prompts', default=3, type=int, help="max prompts per vid")
    # parser.add_argument('--diff_weight', default=1.0, type=float, help="max prompts per vid")
    ...
    parser.add_argument('--pp_threshold', default=0.2, type=float)
    ...
    parser.add_argument('--no_cap_default', default=None, type=str, help="if video is not predicted as unique, default to the initial prompt")
    parser.add_argument('--mode', default="offline", type=str, help="if video is not predicted as unique, default to the initial prompt")
    # parser.add_argument('--freeze_unique', default=1, type=str, help="")
    ...
    parser.add_argument('--comb_maxp', default=2, type=int, help="max cardinality of prompt combinations")
    parser.add_argument('--enforce_amax', default=1, type=int, help="assume the margin is always positive")
    parser.add_argument('--priority', default="tuple", type=str, help="order of preference for going through prompt combinations")
    parser.add_argument('--d_model', default=256, type=int, help="feature dimension")
    ...
    parser.add_argument('--frames', default=4)
    parser.add_argument('--fps', default=1)
    parser.add_argument('--video_size', default=224)
...
class TLMFeatures():
    ...
    def get_vis_embeddings_pp(self, offset=0, set_key=0):
        ...
        # print(offset, self.args.temporal_offset, int(offset / self.args.temporal_offset))
    ...
    def get_vis_embeddings_eval(self, max_offset=0, set_key=0):
        ...
        for i in range(set_size):
            ...
            # vf = [self.feats[set_key][k]['visual'][i]['ViT-L-14'] for k in self.feats[set_key].keys()]
            vf = torch.stack(vf, dim=0)
            all_vf.append(vf)
        vf = torch.stack(all_vf, dim=0).mean(dim=-2)
        return vf
    ...
    def get_text_embedding_eval(self, vid_key=0, offset=0, set_key=0, prompt_id=None):
        ...
        # tf = self.feats[set_key][vid_key]['textual'][int(offset / self.args.temporal_offset)]['text_feature_ViT-L-14'][prompt_id]
        return tf
...
class UniqueCaptioner():
    ...
    def load_embedding_model(self):
        ...
        self.emb_tokenizer = generate_tokenizer(old_args.model)
        ...
        self.emb_val_transform = transforms.Compose([
        ...
        self.emb_val_transform_norm = transforms.Compose([
             (transforms_video.NormalizeVideo(mean=[123.675, 116.28, 103.53], std=[58.395, 57.12, 57.375]) if 'OPENAI' not in old_args.model else
                transforms_video.NormalizeVideo(mean=[108.3272985, 116.7460125, 104.09373615000001], std=[68.5005327, 66.6321579, 70.32316305])),
        ])           
    ...
    def get_frames_only(self, set_k, temporal_offset=0):
        if self.args.dataset == "ego4d":
            ...
            norm_frames = []
            for i in range(cpu_frames.shape[0]):
                norm_frames.append( self.emb_val_transform_norm(cpu_frames[i]) )
            frames = torch.stack(norm_frames).cuda()

        elif self.args.dataset == "tlm":
            raise NotImplementedError("tlm does not support frames only")
        return 0, cpu_frames
    
    def get_text_embeddings(self, texts, vid_key=0, temporal_offset=0, set_key=0):

        if self.args.dataset == "ego4d":
            text_tokens = self.emb_tokenizer(texts).cuda()
            if len(text_tokens.shape) == 1:
                text_tokens = text_tokens.unsqueeze(0)
            text_embs = self.emb_model.encode_text(text_tokens)
        elif self.args.dataset == "tlm":

            print(vid_key, temporal_offset, set_key)

            text_embs = self.tlmfeats.get_text_embedding_eval(vid_key, temporal_offset, set_key).unsqueeze(0).cuda()

        return text_embs
    
    def predict_is_unique(self, vis_emb):
        # if vis_emb.shape[0] != 1:
        threshold = self.args.pp_threshold
        # else:
        #     threshold = 0.0

        # unsqueeze then take [0] at end of function as prompt predictor expects batch.
        data_dict = {"vis_feats": vis_emb.unsqueeze(0)}

        data_dict = self.pp_model(data_dict)

        if "u_cls" in data_dict:
            u_cls = data_dict["u_cls"]
            u_probs = F.softmax(u_cls, dim=-1)[:,:,:,1]
            pred_prompts = torch.argmax(u_probs, dim=-1)
            max_prob_per_vid = torch.max(u_probs, dim=-1)[0]
            is_unique = torch.where(max_prob_per_vid > threshold, 1, 0)
        elif "u_cls_bpv" in data_dict:
            u_cls = data_dict["u_cls_bpv"]
            u_probs = u_cls
            pred_prompts = torch.argmax(u_probs, dim=-2)
            max_prob_per_vid = torch.max(u_probs, dim=-2)[0]
            is_unique = torch.where(max_prob_per_vid > threshold, 1, 0)
        else:
            raise NotImplementedError("prompt predictor output key not recognized")




        return is_unique[0], pred_prompts[0], max_prob_per_vid[0]

    def extract_caption_single(self, vid_keys, prompt_id=None, temporal_offset=0, set_key=0):
    ...
    def extract_captions_batch(self):
        pass
    def run_offline(self, all_vid_keys, all_is_unique, all_pred_prompts, all_pred_probs):

        chosen_offsets = {k: 0 for k in all_vid_keys}
        chosen_prompts = {k: None for k in all_vid_keys}
        is_unique_preds = {k: 0 for k in all_vid_keys}


        max_prob_per_vid, offsets = torch.max(all_pred_probs, dim=0)
        is_unique = torch.where(max_prob_per_vid > self.args.pp_threshold, 1, 0)

        for i, k in enumerate(all_vid_keys):
            if is_unique[i]:
                is_unique_preds[k] = 1
                chosen_offsets[k] = offsets[i].cpu()
                chosen_prompts[k] = all_pred_prompts[offsets[i]][i]

        return is_unique_preds, chosen_offsets, chosen_prompts
    

    def get_metrics(self, is_unique_preds, all_vid_keys, chosen_offsets, chosen_cap_emb, all_vis_emb):
        # get cosine similarity between all vis_embs and the chosen cap emb at the offset of the chosen cap            
        

        cs_cols = []
        for k in all_vid_keys:
            v_e = all_vis_emb[chosen_offsets[k]]
            t_e = chosen_cap_emb[k]
        ...
        metric_dict = {
            ...
            "t_acc": t_acc,
            "vt_acc": vt_acc,
            "vt_pred_only": vt_pred_only,
            "n_unique_attempts": n_u_preds,
            "seconds": self.args.max_offset,
            # "percent_unique": percent_unique,
            # "n_unique_correct": n_unique_correct,
            # "n_unique_attempts": len(unique_preds_correct),
        }
    ...
    def get_metrics2(self, set_k, all_vid_keys, chosen_vis_embs, chosen_cap_embs, chosen_captions, is_unique_overall, chosen_prompts, current_offset, unique_preds_correct):
        percent_unique = float(sum([v for v in is_unique_overall.values()])) / len(all_vid_keys) * 100
        print(f"{current_offset}s, {int(percent_unique)}\%, {set_k}")

        v_e = torch.stack([chosen_vis_embs[k] for k in all_vid_keys])
        t_e = torch.stack([chosen_cap_embs[k] for k in all_vid_keys])

        cs = F.cosine_similarity(v_e.unsqueeze(-3), t_e.unsqueeze(-2), dim=-1).transpose(-1, -2)

        diag_labs = torch.arange(v_e.shape[0])
        v_preds = torch.argmax(cs, dim=-1).cpu()
        t_preds = torch.argmax(cs, dim=-2).cpu()
        v_acc = torch.mean((v_preds == diag_labs).float()) * 100
        t_acc = torch.mean((t_preds == diag_labs).float()) * 100
        ...
        for i in range(v_e.shape[0]):
            if v_preds[i] == i and t_preds[i] == i:
                vt_acc += 1
        vt_acc = vt_acc / v_e.shape[0] * 100

        # unique_pred_acc = sum([v for v in unique_preds_correct.values()]) / len(unique_preds_correct) * 100
        n_unique_correct = sum([v for v in unique_preds_correct.values()])
        ...
        # print(list(is_unique_overall.values()))
        return metric_dict
    ...
    def identify_best_combination(self, all_vid_keys, cs, pp):
        ...
        # return found_unique, unique_idxs
        # return is_unique, unique_idxs
    ...
    def comb_run_set(self, set_k):
        ...
        with torch.no_grad():
            ...
            # all_is_unique = torch.stack(all_is_unique, dim=0)
            # all_pred_prompts = torch.stack(all_pred_prompts, dim=0)
            # all_pred_probs = torch.stack(all_pred_probs, dim=0)
            # print(all_frames[0].shape)
            # offset v c t w h
            all_frames = torch.stack(all_frames, dim=0)
            total_offset = all_frames.shape[0]
            ...
            if self.args.mode == "offline":
                is_unique_preds, chosen_offsets, chosen_prompts = self.comb_run_offline(all_vid_keys, all_cs)
            else:
            ...
            for i, k in enumerate(all_vid_keys):
                if not is_unique_preds[k]:
                    ...
                    elif self.args.no_cap_default == "lav0":
                        chosen_prompts[k] = [None]
                        chosen_offsets[k] = [0]
                    else:
            ...
            for i, k in enumerate(all_vid_keys):
                if len(chosen_cap_emb[k]) == 0:
                    for j in range(len(chosen_prompts[k])):
                        caption = self.extract_caption_single([k], prompt_id=chosen_prompts[k][j], temporal_offset=chosen_offsets[k][j] * self.args.temporal_offset, set_key=set_k)[0]
                        chosen_captions[k].append(caption)

                        cap_emb_single = self.get_text_embeddings(caption, vid_key=k, temporal_offset=chosen_offsets[k][j] * self.args.temporal_offset, set_key=set_k)[0]
            ...
            if self.args.wandb:
                row = []
                for i, k in enumerate(all_vid_keys):
                    # frames = offset v c t w h

                    frames = all_frames[:,i,:,:,:,:]
                    frames = rearrange(frames, 'o c t w h -> (o t) c w h')
                    # frames = rearrange(frames, 'c t w h -> t c w h')
                    ...
                    for o_idx, offset in enumerate(chosen_offsets[k]):
                        # chosen_captions[k][o_idx] = f"{offset}: chosen"
                        wandb_captions.append(f"{offset}: {chosen_captions[k][o_idx]}")

                    # row.append(wandb.Video(frames, fps=args.fps, format="webm", caption=',  '.join(chosen_captions[k])))
            ...
            if self.args.dataset == "ego4d":
                all_vis_emb_eval = all_vis_emb
            elif self.args.dataset == "tlm":
                all_vis_emb_eval = self.tlmfeats.get_vis_embeddings_eval(set_key=set_k, max_offset=self.args.max_offset).cuda()

            pred_dict = {k: {"caption": chosen_captions[k], "offset": chosen_offsets[k]} for k in all_vid_keys}

            metric_dict = self.group_get_metrics(is_unique_preds, all_vid_keys, chosen_offsets, chosen_cap_emb, all_vis_emb_eval)

            print(chosen_prompts)


            return metric_dict, pred_dict, chosen_prompts





    def save_full_vids(self, set_k):
        print(set_k)
        with torch.no_grad():
            if self.args.dataset == "ego4d":
                all_vid_keys = self.emb_val_dataset.sets[set_k]
            ...
            while current_offset * self.args.temporal_offset <= self.args.max_offset:
                # try:
                vis_emb, frames = self.get_frames_only(set_k, current_offset * self.args.temporal_offset)
                all_frames.append(frames)
                current_offset += 1
            current_offset -= 1

            total_offset = len(frames)
            ...
            for i, k in enumerate(all_vid_keys):
                ...
                frames = [f[i] for f in all_frames]
                ...
                # row.append(wandb.Video(frames, fps=args.fps, format="webm", caption=',  '.join(chosen_captions[k])))
                row.append(wandb.Video(frames, fps=args.fps, format="webm", caption=',  '))
            wandb.log({set_k: row})
        return {0:1, 1:2}
...
def main(args):
    ...
    if args.n_sets == 50:
        keys = [k for k in pickle.load(open(osp.join(args.output_dir, "v2-0.5-0-50dl.pkl"), "rb")).keys()]
    else:
        keys.sort()

    # keys = ["#C C climbs down the ladder", 
    #         "#C C looks around a shelf",
    ...
    fprompts = open(f"{fn}_prompts.pkl", 'wb')
    ...
    for i in range(args.n_sets_start, args.n_sets_start + args.n_sets):
        # metric_dict = captioner.run_set(keys[i])
        # metric_dict = captioner.group_run_set(keys[i])
        metric_dict, pred_dict, prompt_dict = captioner.comb_run_set(keys[i])
        # metric_dict, pred_dict = captioner.save_full_vids(keys[i])
        # continue
        ...
        all_prompts_dict[keys[i]] = prompt_dict
    ...
    for k, v in metric_dict_total.items():
        metric_dict_total[k] = v / args.n_sets



    # with open(f"{fn}.pkl", 'wb') as fpkl:
    pickle.dump(log_dict, fpkl)
    pickle.dump(metric_dict_total, fpkl)
    pickle.dump(all_pred_dict, fpreds)
    pickle.dump(all_prompts_dict, fprompts)
    # with open(f"{fn}.txt", 'w') as ftxt:
