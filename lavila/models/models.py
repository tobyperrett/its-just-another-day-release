...
class CLIP(nn.Module):
    ...
    def encode_image(self, image, use_checkpoint=False, apply_project=True, return_features=False):
    ...
    def encode_text(self, text, use_checkpoint=False):
        if len(text.shape) == 3:
            batch_size, variations, n_ctx = text.shape
            multiple_captions = True
            text = text.reshape(-1, n_ctx)
            # text = text[:, 0, :]
        else:
        ...
        if multiple_captions:
            x = x.reshape(batch_size, variations, -1)
            # x = x[:, 0, :]
        return x
    ...
    def forward(self, image, text, use_checkpoint=False, norm_embed=False):
        ...
        #         'text_positive': text_embed[:, 1, :],
        #         'text_negative': text_embed[:, 2, :],
...
class CLIP_HF(nn.Module):
    ...
    # def head_and_project_features(self, x):
...
def get_loss(model, args, tokenizer=None):
    ...
    elif model.startswith('CLIP') or (args.estimator is not None):
...
def get_metric_names(args):
    if (args is None) or args.model.startswith('CLIP') or (args.estimator is not None):
        # return ['loss', 'clip_loss', 'clip_acc', "clip_acc_text", "image_loss", "text_loss", 'clip_loss_s', 'clip_acc_s', "image_loss_s", "text_loss_s", 'clip_loss_t', 'clip_acc_t', "image_loss_t", "text_loss_t", 'clip_loss_b', 'clip_acc_b', "image_loss_b", "text_loss_b", 'clip_loss_bu', 'clip_acc_bu', "image_loss_bu", "text_loss_bu", "u_vid_difference_loss", "assignment_acc"]
        return ['loss', 'clip_loss', 'clip_acc', "clip_acc_text", "image_loss", "text_loss",
                 'clip_loss_u_u', 'clip_acc_u_u', 'image_loss_u_u', 'text_loss_u_u',
                 'clip_loss_o_o', 'clip_acc_o_o', 'image_loss_o_o', 'text_loss_o_o',
                 'clip_loss_b_u_o', 'clip_acc_b_u_o', 'image_loss_b_u_o', 'text_loss_b_u_o',
                 'clip_loss_b_o_u', 'clip_acc_b_o_u', 'image_loss_b_o_u', 'text_loss_b_o_u',
                 "u_vid_difference_loss", "assignment_acc"]
    elif args.model.startswith('VCLM'):
