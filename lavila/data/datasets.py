...
def vr_2_numpy(frames):
    if type(frames) == decord.ndarray.NDArray:
        return frames.asnumpy()
    elif type(frames) == torch.Tensor:
        return frames.numpy()
    else:
        raise NotImplementedError("Type {} not supported for videoreader".format(type(frames)))
...
def video_loader(root, vid, second, end_second=None, chunk_len=300, fps=30, clip_length=32, jitter=False):
    if chunk_len == -1:
        ...
        vr = decord.VideoReader(osp.join(root, '{}.mp4'.format(vid), '{:d}.mp4'.format(int(chunk_start / chunk_len))))
        if len(vr) < 1:
            print(vid)
    ...
    if max(frame_ids) < len(vr):
        try:
            frames = vr_2_numpy(vr.get_batch(frame_ids))
        except decord.DECORDError as error:
            print("+++++" + error)
            print(vid, second, end_second, chunk_len, fps, clip_length, jitter, chunk_start, second_offset, '{}.mp4'.format(vid), '{:d}.mp4'.format(int(chunk_start / chunk_len)), max(frame_ids), min(frame_ids), len(vr))
            try:
                frames = vr_2_numpy(vr.get_batch([0] * len(frame_ids)))
            except error:
                print("......" + error)
        ...
        try:
            ...
            frames_part1 = vr_2_numpy(vr.get_batch(frame_ids_part1))
            vr2 = decord.VideoReader(osp.join(root, '{}.mp4'.format(vid), '{:d}.mp4'.format(int((chunk_start + chunk_len) / chunk_len))))
            ...
            frames_part2 = vr_2_numpy(vr2.get_batch(frame_ids_part2))
...
class VideoCaptionDatasetBase(torch.utils.data.Dataset):
    def __init__(self, dataset, root, metadata, is_trimmed=True):
        ...
        if self.dataset == 'ego4d':
            with open(metadata, 'rb') as f:
                ...
                if type(self.samples) == dict:
                    self.sample_keys = [k for k in self.samples.keys()]
    ...
    def get_raw_item(self, i, is_training=True, num_clips=1, clip_length=32, clip_stride=2, sparse_sample=False,
                     narration_selection='random', index_is_key=False, offset_s=0):
        if self.dataset == 'ego4d':

            if type(self.samples) == dict:
                if index_is_key:
                    cur_dict = self.samples[i]
                else:
                    cur_dict = self.samples[self.sample_keys[i]]

                    
                vid = cur_dict["vid_id"]
                start_second = cur_dict["start"]
                end_second = cur_dict["end"]
                mid_second = cur_dict["timestamp_sec"]

                if self.args.fixed_clip_len > 0:

                    if hasattr(self.args, "extend"):
                        if self.args.extend == "forward":
                            end_second = start_second + self.args.fixed_clip_len
                        elif self.args.extend == "backward":
                            start_second = max(0, end_second - self.args.fixed_clip_len)
                        elif self.args.extend == "both":
                            start_second = max(0, mid_second - self.args.fixed_clip_len/2)
                            end_second = mid_second + self.args.fixed_clip_len/2
                        else:
                            raise NotImplementedError("extend should be forward, backward or both")
                    else:
                        start_second = max(0, mid_second - self.args.fixed_clip_len/2)
                        end_second = mid_second + self.args.fixed_clip_len/2
                    clip_len = self.args.fixed_clip_len
                else:
                    clip_len = end_second - start_second

                start_second = max(0, start_second + offset_s)
                end_second += offset_s

                narration = cur_dict[self.args.dataloader_text] if self.args.dataloader_text in cur_dict.keys() else ""
                positive_list = cur_dict["correct"] if "correct" in cur_dict.keys() else None
                negative_list = cur_dict["incorrect"] if "incorrect" in cur_dict.keys() else None

                frames = video_loader(self.root, vid, start_second,
                                      end_second=end_second,
                                      clip_length=clip_length,
                                      jitter=is_training, chunk_len=self.args.chunk_len)
                ...
                return_list = []
                if self.return_original_narration:
                    return_list.append(narration)

                if self.return_positive:
                    if self.before_after_choice == "random":
                        return_list.append(random.choice(positive_list))
                    elif self.before_after_choice == "before":
                        return_list.append(positive_list[0])
                    elif self.before_after_choice == "after":
                        return_list.append(positive_list[1])
                if self.return_negative:
                    if self.before_after_choice == "random":
                        return_list.append(random.choice(negative_list))
                    elif self.before_after_choice == "before":
                        return_list.append(negative_list[1])
                    elif self.before_after_choice == "after":
                        return_list.append(negative_list[0])
                
                if len(return_list) == 1:
                    return_list = return_list[0]
                    
                return frames, return_list
            else:
                raise NotImplementedError("pkl with list should not be called anymore. Use the dict version instead.")
    ...
    def get_sample_metadata(self, i, i_is_key=False):
        if type(self.samples) == dict:
            i_key = i if i_is_key else self.sample_keys[i]

            sample = {k: v for k, v in self.samples[i_key].items()}
            if "key" not in sample.keys():
                sample["key"] = i_key
            return sample
            # return sample["vid_id"], sample["start"], sample["end"], sample["narration"]

        return self.samples[i]

    def get_set_metadata(self, i, i_is_key=False):
        if self.sets is None:
            raise ValueError("No sets for this dataset")
        i_key = i if i_is_key else self.set_keys[i]

        sample_infos = {k: self.get_sample_metadata(k, i_is_key=True) for k in self.sets[i_key]}
...
class VideoCaptionDatasetCLIP(VideoCaptionDatasetBase):
    def __init__(self, dataset, root, metadata, transform=None,
                 ...
                 return_original_narration=True, return_positive=True, return_negative=True, before_after_choice="random", args = None):
        ...
        self.return_original_narration = return_original_narration
        self.return_positive = return_positive
        self.return_negative = return_negative
        self.before_after_choice = before_after_choice
        self.args = args

        if self.args.load_similar_videos:
            self.sim_types_sampled = {k: 1 for k in self.args.sim_type}

        # print(self.args.metadata_sets)

        self.sets = None

        if (self.args.metadata_sets is not None):
            if not osp.exists(self.args.metadata_sets):
                print("Metadata sets file {} does not exist".format(self.args.metadata_sets))
            else:
        # try:
                print("Loading metadata sets from {}".format(self.args.metadata_sets))
                self.sets = pickle.load(open(self.args.metadata_sets, "rb"))
                if len(self.sets) == 0:
                    self.sets = None
                else:
                    if type(self.sets) == list:
                        self.sets = {i: v for i, v in enumerate(self.sets)}
                    self.set_keys = [k for k in self.sets.keys()]
                    ...
                    dl_key_counter = 0
                    for sk in self.set_keys:
                        new_set = []

                        if type(self.sets[self.set_keys[0]]) == dict:
                            for k, v in self.sets[sk].items():
                                new_samples[dl_key_counter] = v
                                if "key" not in new_samples[dl_key_counter].keys():
                                    new_samples[dl_key_counter]["key"] = k
                                new_samples_keys.append(dl_key_counter)
                                new_set.append(dl_key_counter)
                                dl_key_counter += 1
                        else:
                            for k in self.sets[sk]:
                                new_samples[dl_key_counter] = self.samples[k].copy()
                                if "key" not in new_samples[dl_key_counter].keys():
                                    new_samples[dl_key_counter]["key"] = k
                                new_samples_keys.append(dl_key_counter)
                                new_set.append(dl_key_counter)
                    ...
                    self.sample_keys = new_samples_keys
                    self.sets = new_sets


                    for sk in self.set_keys:
                        for k in self.sets[sk]:
                            self.samples[k]["set_key"] = sk
                print(f"dataset has {len(self.sets)} sets")


    def __get_single_similar_key(self, key, sim_type, already_selected):
        if sim_type == "same-narration-vid":
            same = [k for k in self.samples.keys() if self.samples[k]["narration"] == self.samples[key]["narration"] and k not in already_selected and self.samples[k]["vid_id"] == self.samples[key]["vid_id"]]
            if len(same) > 0:
                return random.choice(same)
            else:
                return None

        if sim_type == "same-narration-ds":
            same = [k for k in self.samples.keys() if self.samples[k]["narration"] == self.samples[key]["narration"] and k not in already_selected]
            if len(same) > 0:
                return random.choice(same)
            else:
                return None

        if sim_type == "caption-embed-vid":
            for sim_i in range(len(self.samples[key]["similar_uids"])):
                if self.samples[key]["similar_uids"][sim_i] not in already_selected:
                    return sim_i
            return None
        
        if sim_type == "temporal-vid":
            same = [k for k in self.samples.keys() if self.samples[k]["vid_id"] == self.samples[key]["vid_id"] and k not in already_selected]
            if len(same) > 0:
                same.sort(key=lambda x: abs(self.samples[x]["timestamp_sec"] - self.samples[key]["timestamp_sec"]))
                return same[0]
            else:
                return None
            
        if sim_type == "same-caption-lavb":
            same = [k for k in self.samples.keys() if self.samples[k]["caption_lav-base"] == self.samples[key]["caption_lav-base"] and k not in already_selected]
            if len(same) > 0:
                return random.choice(same)
            else:
                return None
        ...
        if sim_type == "rand":
            return random.choice(list(self.samples.keys()))

        raise NotImplementedError("Similarity type {} not yet implemented".format(sim_type))


    def __get_similar_keys__(self, key):

        if len(self.args.load_similar_videos_range) != 1:
            raise NotImplementedError("Load_similar_videos_range should be a list of length 1. Variable length is not supported yet.")

        n_to_load = int(self.args.load_similar_videos_range[0])

        already_selected = [key]

        for i in range(n_to_load):
            found = False
            probs = np.array([self.sim_types_sampled[k] for k in self.args.sim_type])
            probs = probs / np.sum(probs)
            sim_attempt_order = np.random.choice(self.args.sim_type, len(self.args.sim_type), p=probs, replace=False)
            for s in sim_attempt_order:
                sim_key = self.__get_single_similar_key(key, sim_type=s, already_selected=already_selected)
                if sim_key is not None:
                    already_selected.append(sim_key)
                    found = True
                    self.sim_types_sampled[s] += 1
                    break
            if not found:
                already_selected.append(random.choice(list(self.samples.keys())))


        return already_selected[1:]


    def __get_single_item__(self, i, index_is_key=False, offset_s=0):
        ...
        if hasattr(self.args, "preprocess_text"):
            if self.args.preprocess_text == "first-sentence-only":
    ...
    def __getitem__(self, i, i_is_key=False, offset_s=0):

        sample_key = i if i_is_key else self.sample_keys[i]
       
        if (not self.args.load_similar_videos) and (self.sets is None):
            return_tuple = self.__get_single_item__(i, offset_s=offset_s)
            return_dict = {"frames": return_tuple[0], "text": return_tuple[1], "relevancy": return_tuple[2], "keys": sample_key}
            return return_dict
            # return return_tuple
        ...
        if self.sets is not None:
            if i_is_key:
                ...
                set_key = self.set_keys[i]

            all_keys = self.sets[set_key]


            # either cap number of similar videos, or pad with random other ones
            n_to_load = int(self.args.load_similar_videos_range[0]) + 1
            if n_to_load > 0:
                if len(all_keys) > n_to_load:
                    all_keys = all_keys[:n_to_load]
                if len(all_keys) < n_to_load:
                    sim_keys = self.__get_similar_keys__(sample_key)
                    all_keys = all_keys + sim_keys
                    all_keys = all_keys[:n_to_load]

            set = [list(self.__get_single_item__(j, index_is_key=True, offset_s=offset_s)) for j in all_keys]
            return_tuple = []
            for i in range(len(set[0])):
                return_tuple.append([e[i] for e in set])
            ...
            sim_keys = self.__get_similar_keys__(sample_key)
            sim_tuples = [self.__get_single_item__(j, index_is_key=True, offset_s=offset_s) for j in sim_keys]

            all_keys = [sample_key] + sim_keys

            return_tuple = [[x] for x in return_tuple]
            for i in range(len(return_tuple)):
                return_tuple[i] = return_tuple[i] + [sim_tuple[i] for sim_tuple in sim_tuples]

        for i in range(len(return_tuple)):
            if isinstance(return_tuple[i][0], torch.Tensor):
                return_tuple[i] = torch.stack(return_tuple[i], dim=0)
            if isinstance(return_tuple[i][0], float):
                return_tuple[i] = np.array(return_tuple[i])
                

        return_dict = {"frames": return_tuple[0], "text": return_tuple[1], "relevancy": return_tuple[2], "keys": all_keys}
...
def get_dataset(train_transform, tokenizer, args, is_training=True, return_original_narration=True, return_positive=True, return_negative=True, before_after_choice="random"):
    ...
    if args.model.startswith('CLIP') or args.model.startswith('VCLM'):
        return VideoCaptionDatasetCLIP(
            ...
            return_original_narration=return_original_narration,
            return_positive=return_positive,
            return_negative=return_negative,
            before_after_choice=before_after_choice,
            args = args
        )
    else:
        raise NotImplementedError
